...
const insert = async () => {
  const mx_Crtd_Dt = await destKnex('candidates').max('wp_created_at', { as: 'mxD' })
  if (!mx_Crtd_Dt[0].mxD) {
    console.log("inside if block")
    mx_Crtd_Dt[0].mxD = 0;
  }
  console.log("---+++++++++++++++-INSERT QUERY--+++++++++++++++--", totalRecords, mx_Crtd_Dt[0].mxD)

  for (let skipRows = 0; skipRows < totalRecords[0].ttl_Rec;) {

    const post_data = await sourceKnex('wp_posts')
      .where({ post_type: "awsm_job_application" })
      .where('post_date', '>', mx_Crtd_Dt[0].mxD)
      ...
      //   // console.log(post.post_modified)
      // }
